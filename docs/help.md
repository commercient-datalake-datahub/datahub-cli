...
- **Reads: prefer GraphQL over REST for anything relational.** Same entities, same key, at `/auth/<tenant>/graphql` — nested reads across declared relationships, filters that traverse into a related entity, `contains` substring search, and `groupBy`/`aggregations` for counts and totals. The REST/OData surface has neither `$count` nor `contains()`, so relying on it alone sends you to raw SQL unnecessarily. Declare relationships (per-entity settings or `set_relationship`), Regenerate once, and a four-way join becomes one request. Reach for the read-only SQL endpoints only past GraphQL's limits: grouping or sorting by a *related* entity's column, nested aggregations, and offset paging.
- **Atomic multi-row writes**: REST and GraphQL writes are **not** transactional, and two GraphQL mutations in one document are not atomic either — a multi-step write can partially succeed. For all-or-nothing, wrap the work in a **stored procedure** (`BEGIN TRAN` / `COMMIT` / `ROLLBACK`), expose it as an entity, and call it as a single `POST`. Procedure entities are POST-only by design, since a write should not be reachable by a "safe" method.
